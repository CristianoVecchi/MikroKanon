...
data class MikroKanon(val parts: List<AbsPart>,val intervalSet: List<Int>,
    ...
    companion object {
        ...
        fun find6AbsPartMikroKanon(
            absPitches: IntArray, intervalSet: IntArray,
            delay1: Int, list2: IntArray,
            delay2: Int, list3: IntArray,
            delay3: Int, list4: IntArray,
            delay4: Int, list5: IntArray,
            delay5: Int, list6: IntArray,
            transpositions: List<Int>, forms: List<Int>,
        ): MikroKanon {
            if (delay1 == 0 || delay2 == 0 || delay3 == 0 || delay4 == 0 || delay5 == 0 ||
                list2.isEmpty() || list3.isEmpty() || list4.isEmpty() || list4.isEmpty() || list5.isEmpty()
            ) {
                return MikroKanon(
                    listOf(AbsPart(absPitches.toMutableList(), delay = 0),),
                    intervalSet.toList()
            ...
            val comes3 = AbsPart(mutableListOf(), RowForm.UNRELATED, delay3)
            val comes4 = AbsPart(mutableListOf(), RowForm.UNRELATED, delay4)
            val comes5 = AbsPart(mutableListOf(), RowForm.UNRELATED, delay5)
            for (i in 0 until delay1) {
                comes1.absPitches.add(-1)
            }
            for (i in 0 until delay2) {
            ...
            for (i in 0 until delay5) {
                comes5.absPitches.add(-1)
            }
            ...
            while (absIndex < absPitches.size) {
                ...
                val comes3Pitch = comes3.absPitches[duxIndex]
                val comes4Pitch = comes4.absPitches[duxIndex]
                val comes5Pitch = comes5.absPitches[duxIndex]

                // dux matching with comites 1 2 3 4 5
                if ((comes1Pitch == -1 || isIntervalInSet(
                        intervalSet,
                        duxPitch,
                        comes1Pitch
                    ...
                    && (comes5Pitch == -1 || isIntervalInSet(
                        intervalSet,
                        duxPitch,
                        comes5Pitch
                    ))
                    // comes1 matching with comites 2 3 4 5
                    && (comes2.absPitches[duxIndex + delay1] == -1 || isIntervalInSet(
                        intervalSet,
                        list2[absIndex], comes2.absPitches[duxIndex + delay1]
                    ))
                    ...
                    && (comes5.absPitches[duxIndex + delay1] == -1 || isIntervalInSet(
                        intervalSet,
                        list2[absIndex], comes5.absPitches[duxIndex + delay1]
                    ))

                    //comes2 matching with comites 3 4 5
                    && (comes3.absPitches[duxIndex + delay2] == -1 || isIntervalInSet(
                        intervalSet,
                        list3[absIndex], comes3.absPitches[duxIndex + delay2]
                    ))
                    ...
                    && (comes5.absPitches[duxIndex + delay2] == -1 || isIntervalInSet(
                        intervalSet,
                        list3[absIndex], comes5.absPitches[duxIndex + delay2]
                    ))

                    //comes3 matching with comes 4 5
                    && (comes4.absPitches[duxIndex + delay3] == -1 || isIntervalInSet(
                        intervalSet,
                        list4[absIndex], comes4.absPitches[duxIndex + delay3]
                    ))
                    && (comes5.absPitches[duxIndex + delay3] == -1 || isIntervalInSet(
                        intervalSet,
                        list4[absIndex], comes5.absPitches[duxIndex + delay3]
                    ))

                    //comes4 matching with comes 5
                    && (comes5.absPitches[duxIndex + delay4] == -1 || isIntervalInSet(
                        intervalSet,
                        list5[absIndex], comes5.absPitches[duxIndex + delay4]
                    ))
                ) {
                    dux.absPitches.add(duxIndex, duxPitch)
                    comes1.absPitches.add(duxIndex + delay1, list2[absIndex])
                    ...
                    comes3.absPitches.add(duxIndex + delay3, list4[absIndex])
                    comes4.absPitches.add(duxIndex + delay4, list5[absIndex])
                    comes5.absPitches.add(duxIndex + delay5, list6[absIndex])
                    absIndex++
                    duxIndex++
                } else {
                    dux.absPitches.add(duxIndex, -1)
                    ...
                    comes3.absPitches.add(duxIndex + delay3, -1)
                    comes4.absPitches.add(duxIndex + delay4, -1)
                    comes5.absPitches.add(duxIndex + delay5, -1)
                    duxIndex++
                }
            }
            return MikroKanon(listOf(dux, comes1, comes2, comes3, comes4, comes5), intervalSet.toList() ,
...
//                                            .count{ pitch -> pitch == -1}
        ...
        suspend fun findAll6AbsPartMikroKanonsParallelReducted(
            context: CoroutineContext,
            absPitches: List<Int>,
            intervalSet: List<Int>,
            depth: Int,
